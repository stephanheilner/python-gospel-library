...
class Test(unittest.TestCase):
    def test_para_html(self):
        ...
        item_package = ItemPackage(item_id=item['id'], item_version=item['version'], session=session)
    ...
    def test_subitem_html(self):
        item = CatalogDB(session=session).item(uri='/scriptures/ot')
        item_package = ItemPackage(item_id=item['id'], item_version=item['version'], session=session)
    ...
    def test_subitems(self):
        item = CatalogDB(session=session).item(uri='/manual/family-finances')
        item_package = ItemPackage(item_id=item['id'], item_version=item['version'], session=session)
        subitems = item_package.subitems()

        self.assertEqual(len(subitems), 1)
        self.assertEqual(subitems[0]['id'], '128026696')
    ...
    def test_subitem(self):
        ...
        self.assertEqual(subitem['id'], '128026696')
    ...
    def test_related_audio_items(self):
        item = CatalogDB(session=session).item(uri='/manual/new-testament-stories')
        item_package = ItemPackage(item_id=item['id'], item_version=item['version'], session=session)
        ...
        self.assertEqual(related_audio_items[0]['id'], 28)
    ...
    def test_related_video_items(self):
        ...
        subitem_id = 128125781

        related_video_items = item_package.related_video_items(subitem_id)

        self.assertEqual(len(related_video_items), 1)
        self.assertEqual(related_video_items[0]['id'], 36)
        self.assertEqual(related_video_items[0]['subitem_id'], str(subitem_id))
        self.assertEqual(related_video_items[0]['video_id'], '1288181761001')
        self.assertEqual(related_video_items[0]['poster_url'], 'https://mediasrv.lds.org/media-services/GA/videoStill/1288181761001')
    ...
    def test_package_related_content_items(self):
        ...
        self.assertEqual(related_content_items[0]['byte_location'], 2658)
